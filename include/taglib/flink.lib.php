...
function lib_flink(&$ctag,&$refObj)
{
    ...
    while($dbrow=$dsql->GetObject())
    {
        ...
        {
            $link = "<li><a href='".$dbrow->url."' target='_blank'>".cn_substr($dbrow->webname,$titlelen)."</a></li>";
        }
        else if($type=='image')
        {
            $link = "<li><a href='".$dbrow->url."' target='_blank'><img src='".$dbrow->logo."' width='88' height='31' border='0'></a></li>";
        }
        else
        {
            if($dbrow->logo=='')
